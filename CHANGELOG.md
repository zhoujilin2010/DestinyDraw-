...
**问题**：智慧选号页面的流程与用户预期不符——奇偶/大小筛选的位置错误、选号模式选项缺失、展示冗余。

**修复**：按用户指定的正确流程重构整个页面。

- **流程调整为**：奇偶/大小筛选（前置）→ 开始自动选号 → 候选号码池（仅合并去重，无分组展示）→ 选号模式（单式/复式/胆拖）→ 开始生成 → 结果
- **单式模式**：支持选一~选十（之前仅选五~选十），新增"生成X组号码"输入
- **复式/胆拖**：也新增"生成X组号码"统一输入
- **生成验证**：新增 `k8MatchScaledRatios` 函数，将20号比例缩放至任意号码数后匹配验证
- **输入同步**：新增 `k8SmartGenCount` 字段的 change 事件绑定
